...
fn simple_transfer(timestamp: u64) -> anyhow::Result<GenerationInputs> {
    ...
    let inputs = GenerationInputs {
        ...
        checkpoint_state_trie_root: H256(hex!(
            "ef46022eafbc33d70e6ea9c6aef1074c1ff7ad36417ffbc64307ad3a8c274b75"
        )),
        block_metadata,
...
fn get_block_proof<F, C, const D: usize>(
    all_circuits: &AllRecursiveCircuits<F, C, D>,
    all_stark: &AllStark<F, D>,
    config: &StarkConfig,
    input1: GenerationInputs,
    input2: GenerationInputs,
    ...
    C::Hasher: AlgebraicHasher<F>,
{
    ...
    let (root_proof1, pv1) =
        all_circuits.prove_root(all_stark, config, input1, &mut timing, None)?;
    timing.filter(Duration::from_millis(100)).print();
    all_circuits.verify_root(root_proof1.clone())?;
    let mut timing = TimingTree::new("prove root second", log::Level::Info);
    let (root_proof2, pv2) =
        all_circuits.prove_root(&all_stark, &config, input2, &mut timing, None)?;
    timing.filter(Duration::from_millis(100)).print();
    all_circuits.verify_root(root_proof2.clone())?;

    let (agg_proof, pv) =
        all_circuits.prove_aggregation(false, &root_proof1, pv1, false, &root_proof2, pv2)?;
    all_circuits.verify_aggregation(&agg_proof)?;

    let (block_proof, _pv) = all_circuits.prove_block(None, &agg_proof, pv)?;
...
fn test_two_to_one_aggregation() -> anyhow::Result<()> {
    ...
    let all_circuits = AllRecursiveCircuits::<F, C, D>::new(
        &all_stark,
        &[16..17, 9..15, 12..17, 14..15, 9..10, 12..13, 17..20],
        &config,
    );

    let block_proof1 = get_block_proof(&all_circuits, &all_stark, &config, inputs0, dummy0)?;
    let block_proof2 = get_block_proof(&all_circuits, &all_stark, &config, inputs1, dummy1)?;

    let final_block_proof1 = all_circuits.prove_block_wrapper(&block_proof1)?;
    all_circuits.verify_block_wrapper(&final_block_proof1)?;
    let final_block_proof2 = all_circuits.prove_block_wrapper(&block_proof2)?;
    all_circuits.verify_block_wrapper(&final_block_proof2)?;

    let agg_block_proof = all_circuits.prove_two_to_one_aggregation(
        false,
        &final_block_proof1,
        false,
        &final_block_proof2,
    )?;
    all_circuits.verify_two_to_one_aggregation(&agg_block_proof)?;

    // Proofs are expected to be independent here so we can safely duplicate them.
    let agg_block_proof2 = all_circuits.prove_two_to_one_aggregation(
        true,
        &agg_block_proof,
        true,
        &agg_block_proof,
    )?;
    all_circuits.verify_two_to_one_aggregation(&agg_block_proof2)
}
