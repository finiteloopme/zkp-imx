...
global remove_accessed_addresses:
    ...
    %read_accounts_linked_list_return_ctr_ptr
    // stack: access_ctr_ptr, retdest
    DUP1 %assert_nonzero
...
global insert_accessed_storage_keys:
    ...
    %insert_slot_return_ptr
    // stack: access_ctr_ptr, cold_access, storage_ptr, addr, slot, retdest
    DUP1 %assert_nonzero
    ...
    MSTORE_GENERAL
    %stack (cold_access, storage_ptr, addr, slot, retdest) -> (storage_ptr, addr, slot, retdest, cold_access, storage_ptr)
    // We only add to the journal if the previous value was not 0.
    %jumpi(add_to_journal)
    %pop2
    JUMP
